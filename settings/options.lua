...
vim.opt.errorfile = "" -- файл ошибок. Куда записать вывод ошибок при :cfile
...
vim.opt.fillchars = {eob = " "} -- символы для заполнения вертикальных разрывов, строк состояния и т.д. По умолчанию настроено нормально
...
vim.opt.guicursor = "n-v-c-sm:block-NormalMode,i-ci:block-InsertMode,r-cr-o:block-OperatorPending,t:block-blinkon500-blinkoff500-TermCursor" -- gui настройки курсора
...
-- vim.opt.langmap = "йцукенгшщзхъэждлорпавыфячсмитьбю;qwertyuiop[]'\\;lkjhgfdsazxcvbnm\\,."
...
vim.opt.ruler = false -- показывать позицию курсора и относительное местоположение в файле. Не используется, т.к. перезаписывается statusline
...
vim.opt.shellpipe = "| tee %s && cat %s" -- после команды :make добавить это и название файла. Название файла подставляется в конец или в %s.
...
vim.opt.shortmess = "lF" -- опции для сообщений от команд
...
vim.opt.updatetime = 1000 -- если столько ms ничего не происходит, то swap файл записывается на диск, также определяет время для CursorHold
